...
class Joint_model(nn.Module):
    def __init__(self, _, hidden_dim, batch_size, max_length, n_class, n_tag, embedding_matrix):
        ...
        self.biLSTM = DynamicLSTM(300, config.hidden_dim // 2, bidirectional=True, batch_first=True,
                                  dropout=config.lstm_dropout, num_layers=1)
        # self.biLSTM = DynamicLSTM(config.hidden_dim, config.hidden_dim // 2, bidirectional=True, batch_first=True,
        #                           dropout=config.lstm_dropout, num_layers=1)
        ...
        # self.gates = nn.ModuleList([nn.Linear(768, 768) for i in range(12)])
        self.key_gates = nn.ModuleList([nn.Linear(768, 768) for i in range(12)])
        self.value_gates = nn.ModuleList([nn.Linear(768, 768) for i in range(12)])


    def forward_logit(self, x, mask, input_ids, token_type_ids, attention_mask, device):
        ...
        #     return_dict=True
        # )
        # seq_emb = seq_output['last_hidden_state']
        # seq_emb_len = torch.sum(input_ids != 0, dim=-1)
        x, x_char = x
        x_len = torch.sum(x != 0, dim=-1)
        x_emb = self.emb_drop(self.embed(x))
        H, (_, _) = self.biLSTM(x_emb, x_len.to('cpu'))
        # H, (_, _) = self.biLSTM(seq_emb, seq_emb_len.to('cpu'))
        # H_I, H_S = self.I_S_Emb(seq_emb, seq_emb, attention_mask)
        H_I, H_S = self.I_S_Emb(H, H, attention_mask)
        ...
        # I_S = torch.cat((H_I, H_S), dim=2)
        ########################
        prompt_guids = []
        for idx in range(12):
            key_temp = F.leaky_relu(self.key_gates[idx](H_I))
            value_temp = F.leaky_relu(self.value_gates[idx](H_S))
            key = key_temp.reshape(bsz, 12, -1, 64).contiguous()
            value = value_temp.reshape(bsz, 12, -1, 64).contiguous()
            # key_val_temp = F.leaky_relu(self.gates[idx](I_S))
            # key_val_temp = F.softmax(F.leaky_relu(self.gates[idx](H_I)))
            ...
            #############
            # key_temp = F.softmax(F.leaky_relu(self.key_gates[idx](H_I)))
            # value_temp = F.softmax(F.leaky_relu(self.value_gates[idx](H_S)))
            # key = key_temp.reshape(bsz, 12, -1, 64)
            # value = value_temp.reshape(bsz, 12, -1, 64)
            #############
            temp_dict = (key, value)
            prompt_guids.append(temp_dict)
        prompt_guids_length = prompt_guids[0][0].shape[2]
        prompt_guids_mask = torch.ones((bsz, prompt_guids_length))
        prompt_guids_mask = prompt_guids_mask.to(device)
        ...
        ########################
        cls_output = output['last_hidden_state'][:, 0, :]  # Get the embedding of the [CLS] token
        logits_intent = self.intent_fc(cls_output)
        sequence_output = output['last_hidden_state']
        sequence_output = self.dropout(sequence_output)
        logits_slot = self.slot_fc(sequence_output)
...
class Label_Attention(nn.Module):
    ...
    def forward(self, input_intent, input_slot, mask):
        ...
        intent_probs = F.leaky_relu(intent_score)
        slot_probs = F.leaky_relu(slot_score)
